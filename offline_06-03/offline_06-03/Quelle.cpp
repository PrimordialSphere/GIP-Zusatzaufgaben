...
void linie(int x1, int y1, int x2, int y2, char canvas[][canvas_size]) {
	if ((x1-x2==-1 && y1-y2==1)|| (x1 - x2 == -1 && y1 - y2 == -1) || (x1 - x2 == 1 && y1 - y2 == 1) || (x1 - x2 == 1 && y1 - y2 == -1) || (x1 - x2 == -1 && y1 - y2 == 0) || (x1 - x2 == 1 && y1 - y2 == 0) || (x1 - x2 == 0 && y1 - y2 == -1) || (x1 - x2 == 0 && y1 - y2 == 1)) {
		if ((x1 >= 0 && x1 < canvas_size) && (y1 >= 0 && y1 < canvas_size)) { canvas[x1][y1] = filled_pixel; }
		else { cout << "failure" << endl; }
		if ((x2 >= 0 && x2 < canvas_size) && (y2 >= 0 && y2 < canvas_size)) {
